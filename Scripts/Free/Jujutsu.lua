            ...
            if myChar then
                ...
                if myRoot then
                    ...
                    for _, p in ipairs(playersList) do
                        if p and not isLocalPlayer(p) and p.Character then
                    ...
                    if blockAnimPlayer and not blockTriggered then
                        blockTriggered = true
                        blockStart = tick()
                        keypress(KEY_F)
                    end
...
task.spawn(function()
    while true do
        ...
        if _G.AutoBlockDash_Enabled then
            local myChar = LocalPlayer and LocalPlayer.Character
            if myChar then
                local myRoot = myChar and (myChar:FindFirstChild("HumanoidRootPart") or myChar:FindFirstChild("Torso") or myChar:FindFirstChild("UpperTorso"))
                if myRoot then
                    ...
                    if dashAnimPlayer and not dashTriggered then
                        dashTriggered = true
                        dashStart = tick()
                        keypress(KEY_F)
                    end
