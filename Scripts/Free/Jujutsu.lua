...
local VISIBLE_OFFSET = 1461  -- was 1445
...
local KnownOffsets = {
    ...
    Name                     = 176,
    ActiveAnimations         = 2152, -- was 2136
    NodeNext                 = 16,
    Animation                = 208,
}
...
task.spawn(function()
    while true do
        task.wait()
        if not _G.AutoBlock_Enabled then
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            ...
            if getMagnitude(theirRoot.Position, myRoot.Position) <= BLOCK_RANGE then
...
task.spawn(function()
    while true do
        task.wait()
        if not _G.AutoBlockDash_Enabled then
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            ...
            local theirRoot = p.Character:FindFirstChild("HumanoidRootPart")
                           ...
                           or p.Character:FindFirstChild("UpperTorso")
            if not theirRoot then continue end
            if getMagnitude(theirRoot.Position, myRoot.Position) <= DASH_RANGE then
