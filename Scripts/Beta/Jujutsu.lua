    ...
    for _, character in ipairs(characters:GetChildren()) do
        ...
        if ratio then
            ...
            if owner and owner.Value then
                ...
                if ok and ownerName == myName then return character, ratio end
            end
        end
    end
    return nil, nil
...
local function ProcessRatioQTE()
    ...
    if rPressed and not RatioState.rWasPressed then
        RatioState.isScanning = true RatioState.scanStartTime = currentTime
        RatioState.trackedTarget = nil RatioState.trackedRatioValue = nil RatioState.hasTriggered = false
    end
    ...
    if RatioState.isScanning then
        ...
        if target and ratioValue then
            RatioState.isScanning = false RatioState.trackedTarget = target
            RatioState.trackedRatioValue = ratioValue RatioState.qteDetectedTime = currentTime
            RatioState.hasTriggered = false RatioState.targetHealthPercent = GetTargetHealthPercent(target)
    ...
    if RatioState.trackedRatioValue then
        if not RatioState.trackedRatioValue.Parent or RatioState.trackedRatioValue:GetAttribute("Activated") == false then
            RatioState.trackedTarget = nil RatioState.trackedRatioValue = nil RatioState.hasTriggered = false return
        end
        ...
        if currentTime - RatioState.qteDetectedTime >= RatioState.calculatedDelay then
            ...
            RatioState.hasTriggered = true RatioState.lastTriggerTime = currentTime
        end
    end
end
...
local function ResetSwapState()
    SwapState.isScanning = false SwapState.swapConfirmed = false
    SwapState.effectsSnapshot = {} SwapState.hasTriggeredM1 = false
end
...
local function ProcessPerfectSwap()
    ...
    if rPressed and not SwapState.rWasPressed then
        SwapState.isScanning = true SwapState.scanStartTime = currentTime
        SwapState.swapConfirmed = false SwapState.hasTriggeredM1 = false SwapState.effectsSnapshot = {}
    end
    ...
    if not SwapState.swapConfirmed then
        if HasClapTarget() then
            ...
            if effectsFolder then for _, desc in ipairs(effectsFolder:GetDescendants()) do SwapState.effectsSnapshot[desc.Address] = true end end
        end
        return
    end
    local effectsFolder = workspace:FindFirstChild("Effects")
            ...
            if not SwapState.effectsSnapshot[desc.Address] then
                if desc.ClassName == "MeshPart" and desc.Name == "Clap" then
                    if not SwapState.hasTriggeredM1 and not HasNoM1Flag() then
                        mouse1press() mouse1release() SwapState.hasTriggeredM1 = true
                    end
...
local function CreateESPDrawing(name, color)
    ...
    drawing.Font = Drawing.Fonts.System drawing.Text = name
    drawing.Color = Color3.fromRGB(color[1], color[2], color[3])
    drawing.Outline = true drawing.Center = true drawing.Visible = false
    return drawing
end
...
local function UpdateESPPositions()
    if not CONFIG.esp.enabled then
        for _, espObj in pairs(ESPState.objects) do if espObj.drawing then espObj.drawing.Visible = false end end return
    end
...
local function DiscoverDummy()
    ...
    if ESPState.objects[dummyKey] then
        ...
        if not espObj.part or not espObj.part.Parent then RemoveESPDrawing(espObj) ESPState.objects[dummyKey] = nil else return end
    end
    local dummy = SafeFind(workspace, "Characters", "Dummy") if not dummy then return end
    local hrp = dummy:FindFirstChild("HumanoidRootPart") if not hrp then return end
    ESPState.objects[dummyKey] = { drawing = CreateESPDrawing("Dummy", CONFIG.esp.colorDummy), instance = dummy, part = hrp, type = "dummy" }
end

local function DiscoverItems()
    local itemsFolder = SafeFind(workspace, "Items") if not itemsFolder then return end
    local seenAddresses = {}
    for _, item in ipairs(itemsFolder:GetChildren()) do
        if item:IsA("Part") or item:IsA("MeshPart") then
            local address = item.Address seenAddresses[address] = true
            if not ESPState.objects[address] then
                ESPState.objects[address] = { drawing = CreateESPDrawing(item.Name, CONFIG.esp.colorItems), instance = item, part = item, type = "item" }
            end
        end
    end
    for address, espObj in pairs(ESPState.objects) do
        if espObj.type == "item" and not seenAddresses[address] then RemoveESPDrawing(espObj) ESPState.objects[address] = nil end
    end
end
...
local function CreateDomainHealthDrawing()
    local d = Drawing.new("Text") d.Font = Drawing.Fonts.System d.Size = 16
    d.Color = Color3.fromRGB(255, 255, 255) d.Outline = true d.Center = true d.Visible = false
    return d
end

local function UpdateDomainHealthESP()
    if not CONFIG.domainHealthESP.enabled then
        for _, entry in pairs(DomainHealthESPState.drawings) do entry.drawing.Visible = false end return
    end
    ...
    if domainsFolder then
        for _, domain in ipairs(domainsFolder:GetChildren()) do
            ...
            if domainMesh then
                local addr = domainMesh.Address seenAddresses[addr] = true
                local entry = DomainHealthESPState.drawings[addr]
                if not entry then entry = { drawing = CreateDomainHealthDrawing() } DomainHealthESPState.drawings[addr] = entry end
                ...
                if dist <= nearbyThreshold then
                    entry.drawing.Position = Vector2.new(screenCenterX, 50) entry.drawing.Visible = true
                else
    ...
    for addr, entry in pairs(DomainHealthESPState.drawings) do
        if not seenAddresses[addr] then entry.drawing:Remove() DomainHealthESPState.drawings[addr] = nil end
    end
end
...
local function FindPlayerDomain()
    local domainsFolder = workspace:FindFirstChild("Domains") if not domainsFolder then return nil end
    ...
    for _, domain in ipairs(domainsFolder:GetChildren()) do
        ...
        if domainMesh then
            ...
            if checkPart and checkPart:IsA("BasePart") then
                local distance = CalculateDistance(playerPos, checkPart.Position)
                if distance < closestDistance then closestDistance = distance closestDomain = domainMesh end
            end
        end
    end
...
local function CreateVoteDrawings()
    local function createDrawing()
        local d = Drawing.new("Text") d.Font = Drawing.Fonts.System d.Text = "0" d.Size = 24
        d.Color = Color3.fromRGB(255, 0, 0) d.Outline = true d.Center = true d.Visible = false return d
    end
...
local function ResetDomainState()
    DomainState.isActive = false DomainState.trackedDomain = nil DomainState.waitingForChooseUI = false
    DomainState.waitStartTime = 0 DomainState.confessCount = 0 DomainState.silenceCount = 0 DomainState.denialCount = 0
    HideVoteDrawings()
end
...
local function ProcessDomainVotes()
    ...
    if gPressed and not DomainState.gWasPressed and not DomainState.isActive and not DomainState.waitingForChooseUI then
        DomainState.waitingForChooseUI = true DomainState.waitStartTime = currentTime
    end
    ...
    if DomainState.waitingForChooseUI then
        ...
        if HasChooseUI() then
            ...
            if domain then DomainState.isActive = true DomainState.trackedDomain = domain CreateVoteDrawings() PositionVoteDrawings() UpdateVoteDisplay(0, 0, 0) end
        end
        return
    end
    if DomainState.isActive then
        ...
        if confess ~= DomainState.confessCount or silence ~= DomainState.silenceCount or denial ~= DomainState.denialCount then
            DomainState.confessCount = confess DomainState.silenceCount = silence DomainState.denialCount = denial
...
local KnownOffsets = {
    AnimationId = 208, ClassDescriptor = 24, ClassDescriptorToClassName = 8,
    Name = 176, ActiveAnimations = 2136, NodeNext = 16, Animation = 208,
}
...
local function readStr(addr)
    ...
    local s, str = pcall(function() return memory_read("string", addr) end)
    return s and str or nil
end

local function readPtr(addr)
    local s, val = pcall(function() return memory_read("uintptr_t", addr) end)
    if s and val and val > 4096 then return val end
    return nil
end
...
local function getClass(addr)
    ...
    if desc then
        ...
        return readStr(namePtr) or "Unknown"
    end
    return "Error"
end
...
local function getCurrentAnimFromChar(char, mode)
    ...
    local curr = first
    while curr and curr ~= 0 and curr ~= head do
...
local function isUltimateReady()
    ...
    local readyFrame = playerGui 
        and playerGui:FindFirstChild("Main")
        and playerGui.Main:FindFirstChild("Ultimate")
        and playerGui.Main.Ultimate:FindFirstChild("Bar")
        and playerGui.Main.Ultimate.Bar:FindFirstChild("Ready")
    if readyFrame then
        local addr = tonumber(readyFrame.Address)
        if addr and addr > 4096 then
            local ok, isVisibleByte = pcall(memory_read, "byte", addr + VISIBLE_OFFSET)
            if ok then return (isVisibleByte ~= 0) end
        end
    end
    return false
end

local function checkForHeadOfHei(char)
    if not char then return false end
    local moveset = char:FindFirstChild("Moveset")
    return moveset and moveset:FindFirstChild("Projection Breaker") ~= nil
end

local function checkForCharles(char)
    if not char then return false end
    local moveset = char:FindFirstChild("Moveset")
...
local function checkEquippedAbilities()
    ...
    local moveset = main:FindFirstChild("Moveset")
...
local function monitorCharlesCooldown()
    ...
    local eyeCatchingFrame = moveset and moveset:FindFirstChild("Eye Catching")
    if not eyeCatchingFrame then return true end
    local cooldown = eyeCatchingFrame and eyeCatchingFrame:FindFirstChild("Cooldown")
    ...
    if secondsLeft and secondsLeft == secondsLeft then
...
DefSec:AddToggle("Auto Counter", false, function(v)
    ...
    if v then task.spawn(function() wait(0.5) checkEquippedAbilities() end) end
end)
...
RatioSec:AddToggle("Auto Ratio QTE", false, function(v)
    ...
    if not v then RatioState.trackedTarget = nil RatioState.trackedRatioValue = nil RatioState.isScanning = false RatioState.hasTriggered = false end
end)
...
DomainHPSec:AddToggle("Domain Health ESP", false, function(v)
    ...
    if not v then for _, entry in pairs(DomainHealthESPState.drawings) do entry.drawing.Visible = false end end
end)
...
task.spawn(function()
    while true do
        ...
        if _G.AutoCounter_Enabled and charlesEquipped then charlesReady = monitorCharlesCooldown() end
...
task.spawn(function()
    while true do
        if _G.AutoCounter_Enabled or _G.AwakeningCounter_Enabled then checkEquippedAbilities() end
        wait(1)
    end
end)
    ...
    while running do
        if CONFIG.autoQTE.enabled and not iskeypressed(KEYS.F) then
            if ProcessQTE() then
                ...
                if CONFIG.autoQTE.deviation > 0 then delay = delay + (math.random() * 2 - 1) * CONFIG.autoQTE.deviation end
                task.wait(delay)
...
spawn(function() while running do pcall(UpdateESPPositions) pcall(UpdateDomainHealthESP) task.wait(1 / math.max(1, CONFIG.esp.updateRate)) end end)
